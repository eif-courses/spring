...
$.getJSON("tasks/java/tasks.json", function (data) {
    ...
    $.each(data, function (key, val) {
        ...
        $("#Pratybos").append("<hr><button class='accordion'><b>" + (i + 1) + " užduotis.</b> (" + m.format('YYYY-MM-DD') + " iki " + d.format('YYYY-MM-DD') + "). " +
            val.head + "</button><div class='panel'>" +"<ul class='w3-ul' id='uzdjava"+i + "'>"+"<li class='w3-padding-16'>"+"<span class='nuoroda'>Tikslas!<br/></span>" +val.aim + "</li></ul></div>");
        $(".accordion").click(function () {
            this.classList.toggle("active");
            this.nextElementSibling.classList.toggle("show");
        });
        ...
        $.each(val.tasks, function (k, v) {
            $("#uzdjava"+i).append("<li class='w3-padding-16'>"+"<img src='images/tasks.png' class='w3-left w3-circle w3-margin-right' style='width:50px'><span>"+ v + "</span></li>");
        });

        $("#uzdjava"+i).append("<li class='w3-padding-16'>"+"<span class='nuoroda'>Pavyzdys!<br/></span>"+"<span>"+ val.example + "</span></li>");
        i++;
    });
});
